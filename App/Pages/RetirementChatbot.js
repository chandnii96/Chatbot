...
export default function RetirementChatbot({ navigation }) {
  ...
  const [isRetirementPlanCompleted, setIsRetirementPlanCompleted] = useState(false);
  ...
  const steps = [
    ...
    { question: 'What are your main goals for your retirement?', field: 'retirementGoals' },
  ...
  const generateRetirementPlan = async (responses) => {
    try {
      const plan = await FinancialService.getRetirementPlan(responses);
      const planMessage = {
        _id: Math.random() * (9999999 - 1),
        text: plan,
        createdAt: new Date(),
