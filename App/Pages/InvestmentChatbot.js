...
const chatbotAvatar = 'https://res.cloudinary.com/dknvsbuyy/image/upload/v1685678135/chat_1_c7eda483e3.png';

export default function InvestmentChatbot({ navigation }) {
  const [messages, setMessages] = useState([]);
  const [currentStep, setCurrentStep] = useState(0);
  const [isInvestmentPlanCompleted, setIsInvestmentPlanCompleted] = useState(false);
  ...
  const steps = [
    ...
    { question: 'How much money have you saved for investments?', field: 'savedMoney' },
    { question: 'What is your risk tolerance?', field: 'riskTolerance', options: ['low', 'medium', 'high'] },
    { question: 'What are your main investment goals?', field: 'investmentGoals' },
    { question: 'What is your investment horizon?', field: 'investmentHorizon', options: ['short-term', 'long-term'] },
    { question: 'What is your current income level?', field: 'incomeLevel' },
    { question: 'What are your expenses and liabilities?', field: 'expensesAndLiabilities' }
  ];
  ...
  const askNextQuestion = (stepIndex) => {
    if (stepIndex < steps.length) {
      ...
      if (step.options) {
        ...
        setMessages(previousMessages => GiftedChat.append(previousMessages, [optionsMessage]));
      }
    } else {
  ...
  const generateInvestmentPlan = async (responses) => {
    try {
      const plan = await FinancialService.getInvestmentPlan(responses);
      const planMessage = {
        _id: Math.random() * (9999999 - 1),
        text: plan,
        createdAt: new Date(),
